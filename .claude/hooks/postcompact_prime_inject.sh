...
# Inject AgentCore agent mesh reminder
cat <<'AGENTS'

--- GALDERMA TRACKWISE AGENT MESH (9 AGENTS) ---
| Agent                  | Model         | Purpose                              |
| ---------------------- | ------------- | ------------------------------------ |
| Observer               | Claude Haiku  | TrackWise event detection            |
| Case Understanding     | Claude Haiku  | Complaint/Inquiry extraction         |
| Recurring Detector     | Claude Haiku  | Pattern recognition + memory lookup  |
| Compliance Guardian    | Claude OPUS   | Policy enforcement (critical)        |
| Resolution Composer    | Claude OPUS   | Multi-language output (critical)     |
| Inquiry Bridge         | Claude Haiku  | Inquiry→Complaint linkage            |
| Writeback              | Claude Haiku  | TrackWise simulator updates          |
| Memory Curator         | Claude Haiku  | STM/LTM memory operations            |
| CSV Pack               | Claude Haiku  | Validation artifacts (21 CFR Part 11)|

A2A Protocol: JSON-RPC on port 9000 via AgentCore Gateway
AGENTS
